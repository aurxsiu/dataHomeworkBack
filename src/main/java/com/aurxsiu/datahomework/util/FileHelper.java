    ...
    private static void getNodeMapScenic() throws Exception{
        ...
        ArrayList<Node> nodeList = nodes.stream().map(v -> {
            ...
            if (index < buildingsNum) {
                return new Node(v.x, v.y, buildings[index], v.id);
            } else if (index < buildingsNum + restNum) {
                return new Node(v.x, v.y, rests[index - buildingsNum], v.id);
            } else if (index < buildingsNum + restNum + storeNum) {
                return new Node(v.x, v.y, stores[index - buildingsNum - restNum], v.id);
            } else {
                return new Node(v.x, v.y, "WC" + (char)('A' + index - buildingsNum - restNum - storeNum), v.id);
            }
    ...
    private static void getNodeMapSchool() throws Exception{
        ...
        ArrayList<Node> collect = nodes.stream().map(v -> {
            ...
            if (index < jiaoxuelouNum) {
                return new Node(v.x, v.y, "教学楼" + (jiaoxuelouNum - index), v.id);
            } else if (index < jiaoxuelouNum + bangonglouNum) {
                return new Node(v.x, v.y, "办公楼" + (bangonglouNum + jiaoxuelouNum - index), v.id);
            } else if (index < jiaoxuelouNum + bangonglouNum + sushelouNum) {
                return new Node(v.x, v.y, "宿舍" + (sushelouNum + jiaoxuelouNum - bangonglouNum - index), v.id);
            } else if (index < jiaoxuelouNum + bangonglouNum + sushelouNum + wcNum) {
                return new Node(v.x, v.y, "WC" + (wcNum + jiaoxuelouNum + bangonglouNum + sushelouNum - index), v.id);
            } else {
                return new Node(v.x, v.y, "食堂" + (jiaoxuelouNum + bangonglouNum + sushelouNum + wcNum + restNum - index), v.id);
            }
        }).sorted(Comparator.comparing(Node::getId)).collect(Collectors.toCollection(ArrayList::new));
